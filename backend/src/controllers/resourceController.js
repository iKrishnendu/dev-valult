...
const createResource = asyncHandler(async (req, res) => {
  ...
  if (!title?.trim() || !url?.trim() || !type || !categoryId) {
    return res.status(400).json({ message: 'Resource title, url, type, and category are required.' })
  }
...
const updateResource = asyncHandler(async (req, res) => {
  ...
  if (isBookmarked !== undefined) nextValues.isBookmarked = Boolean(isBookmarked)
